...
✅ Прогресс: скачивание — в консоли, отправка в Telegram — в консоли и в чате  
✅ Скачивает только само видео, даже если в ссылке есть `list=` — плейлист не тянется  
...
- Windows 10 / 11 (на Linux тоже работает: `python3`, `ffmpeg` и `deno` ставятся из пакетов дистрибутива)  
...
> 💡 **Необязательно:** `tgcrypto` ускоряет шифрование MTProto в несколько раз. На Python 3.14 оригинальный пакет не собирается без Visual C++ Build Tools — вместо них попробуйте форк: `py -m pip install tgcrypto2`. Без него бот тоже работает, просто отправляет медленнее.
...
```cmd
py -m pip install kurigram yt-dlp
```

> ❗ Если при установке появится предупреждение вида  
> `WARNING: The script yt-dlp.exe is installed in '...\Scripts' which is not on PATH` —  
> для работы бота оно **не важно**: бот использует yt-dlp как модуль Python. Добавлять `Scripts` в `PATH` нужно только если хотите запускать `yt-dlp` из командной строки вручную.  
> Путь обычно такой: `C:\Users\<имя>\AppData\Local\Python\pythoncore-<версия>-64\Scripts`.  
> Альтернатива — запускать через `py -m yt_dlp`.
...
4. Проверьте `.gitignore` в корне проекта — там не должно быть в репозитории:

```gitignore
mediaLoaderBot.session*
cookies.txt
downloads/
__pycache__/
```

> Сессия — это доступ к боту, cookies — доступ к вашим аккаунтам.
...
Бот запустится и начнёт принимать ссылки. Отправьте ему URL в личные сообщения — и через пару минут получите видео до 1080p (прогресс отправки виден в чате, весь прогресс — в консоли).

- Файлы сохраняются в папку `downloads` рядом со скриптом, имя вида `Заголовок [id].mp4` (заголовок обрезается до 80 символов — защита от слишком длинных путей). `.txt` с комментариями получает то же имя.
...
- **`'yt-dlp' не является внутренней или внешней командой`** → добавьте папку `Scripts` в `PATH` (см. шаг 4) и откройте **новое** окно терминала. Уже открытые окна не видят изменений `PATH` (вкладки Windows Terminal закрывайте целиком). Боту это не нужно — только для ручного запуска yt-dlp.
- **`ffmpeg -version` не находится** → проверьте, что путь к `bin` попал в `PATH`, и перезапустите терминал.
- **Ошибка извлечения видео с YouTube** → чаще всего помогает обновление: `py -m pip install -U yt-dlp`. YouTube меняет защиту постоянно, старые версии yt-dlp — причина №1 ошибок.
...
- **`TgCrypto is missing!`** → это не ошибка, бот работает. Для ускорения: `py -m pip install tgcrypto2` (оригинальный `tgcrypto` на 3.14 требует Visual C++ Build Tools).
- **`Sign in to confirm you're not a bot`** (YouTube) → положите свежий `cookies.txt` рядом со скриптом и повторите.
- **Комментарии не приходят в `.txt`** → обновите yt-dlp (`py -m pip install -U yt-dlp`): механизм обхода YouTube (`remote_components`) активно развивается. На некоторых видео комментариев просто нет — тогда файл и не создаётся.
